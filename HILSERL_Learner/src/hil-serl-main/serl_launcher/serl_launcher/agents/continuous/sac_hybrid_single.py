...
class SACAgentHybridSingleArm(flax.struct.PyTreeNode):
    ...
    @partial(jax.jit, static_argnames=("argmax", "grasp_eps"))
    def sample_actions(
        self,
        observations: Data,
        *,
        ...
        **kwargs,
        ...
        ``grasp_eps``: when >0, epsilon-greedy on the discrete gripper. Exploration
        samples only from {grasp, release} (not hold), so a collapsed "always 0"
        critic still tries open/close during HIL training. Use 0 for eval.
        """
        ...
        # Select grasp actions based on the grasp Q-values (indices 0,1,2 → -1,0,+1)
        greedy = grasp_q_values.argmax(axis=-1)
        if float(grasp_eps) > 0.0:
            seed, eps_key, rand_key = jax.random.split(seed, 3)
            # Bias explore toward open/close; hold is already the collapsed default.
            random_a = jax.random.choice(
                rand_key, jnp.asarray([0, 2], dtype=jnp.int32), shape=greedy.shape
            )
            explore = jax.random.uniform(eps_key, shape=greedy.shape) < float(grasp_eps)
            grasp_action = jnp.where(explore, random_a, greedy)
        else:
            grasp_action = greedy
