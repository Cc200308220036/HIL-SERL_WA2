#!/usr/bin/env bash
# Generic WA2 learner launcher. Usage:
#   bash run_learner.sh bottle_pick --dry-run
# Formal: bash run_learner.sh bottle_pick --checkpoint_path=... --demo_path=... --debug
set -euo pipefail
...
cd "$EXAMPLES"
exec python train_rlpd.py --exp_name="$EXP_NAME" --learner "${FORWARD[@]}"
